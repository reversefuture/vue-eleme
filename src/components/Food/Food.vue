...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    mounted(){
      this.$help.$on('ratingtype.select', (payload) => {
        this.selectType = payload.type
        this.$nextTick(() => {
          // this.scroll.refresh()
        })        
      })
      this.$help.$on('content.toggle', (payload) => {
        this.onlyContent = payload.onlyContent;
        this.$nextTick(() => {
          // this.scroll.refresh();
        })        
      })
    ...
    methods: {
      show() { //called in parent Good.vue
        this.showFlag = true;
        this.selectType = ALL;
      ...
      needShow(type, text) {
        if (this.onlyContent && !text) { //只显示有内容且内容不为空
          return false;
        }
